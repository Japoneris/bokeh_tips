...
Assume you CANNOT rotate the panel (worst case, but realistic)
"""

from bokeh.plotting  import ColumnDataSource, figure, output_file, show
from bokeh.models    import Segment, HoverTool, CustomJS
from bokeh.models    import Slider, RangeSlider
from bokeh.layouts   import column, row
from bokeh.palettes  import Plasma256, Turbo256, Inferno256
...
if __name__ == "__main__":
    ...
    hra    = d2r(15 * (hours - 12)) 
    beta_range = np.arange(1, 90) # Exclude zero because we get an error otherwise.
    
    beta_range_rad = d2r(beta_range)
    tb = np.tan(beta_range_rad)
    ...
    for N in range(365):
        ...
        for h in hra:
            ang = np.arcsin(A + B * np.cos(h))
            if ang > 0:
                zs = np.sqrt(1 + np.tan(ang)**2 + np.tan(h)**2)
            
                yields += (1 + np.tan(ang) / tb).clip(0).sum() / (zs * zp)
            
    yields = yields / (len(hra) * 365 * len(beta_range)) * 100
    ...
    output_file("html/sun_yield_year_fixed.html", title="Average yield over the year.")
