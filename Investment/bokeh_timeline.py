...
if __name__ == "__main__":
    ...
    def savings(timing):
        ...
        np_dividends = np.zeros(len(timing))
        np_capital  = np.zeros(len(timing))
        ...
        for i, time in enumerate(timing):
            if (time < t1):
                savings = invested0 + (saving - fee_per_year)*time
                dividends = (invested0 + savings)/2 * time * dv
                left = left_to_pay - time * salary
                
                np_dividends[i] = dividends
                np_capital[i] = savings
            
            else:
                savings0 = invested0 + (saving - fee_per_year) * t1
                savings1 = savings0  + (salary + saving)*(time - t1)
                
                dividends0 = (invested0 + savings0)/2 * t1 * dv
                dividends1 = (savings1 + savings0)/2 * (time - t1) * dv
                
                np_dividends[i] = dividends0 + dividends1
                np_capital[i] = savings1
    
        return np_capital, np_dividends

    y_capital, y_dividend = savings(x_range)
    
    source   = ColumnDataSource(data=dict(x=x_range, y=y_capital+y_dividend, y_div=y_dividend, y_cap=y_capital))
    ...
    callback = CustomJS(args=dict(source=source,
        ...
        for (var i=0; i < data.x.length; i++) {
            ...
            if (time < t1) {
                var savings = invested0 + (saving - fee_per_year)*time;
                var dividends = (invested0 + savings)/2 * time * divs;
                var left = left_to_pay - time * salary;
                
                data.y_div[i] = dividends;
                data.y_cap[i] = savings;
                data.y[i]     = savings + dividends;
            
            } else {
                var savings0 = invested0 + (saving - fee_per_year) * t1;
                var savings1 = savings0  + (salary + saving)*(time - t1);
                
                var dividends0 = (invested0 + savings0)/2 * t1 * divs;
                var dividends1 = (savings1 + savings0)/2 * (time - t1) * divs;
                
                data.y_div[i] = dividends0 + dividends1;
                data.y_cap[i] = savings1;
                data.y[i]     = data.y_cap[i] + data.y_div[i];
    ...
    p.line('x', 'y_cap', source=source, line_width=2, line_alpha=0.7, color="red", legend_label="Investment Capital")
    ...
    output_file("output/investment_temporal.html", title="Money invested.")
